...
class SearchResult implements ISearchResult
{
    ...
    public function getFilters()
    {
        ...
        foreach($this->raw->filters->filter as $rawFilter) {
            $filters[] = new Filter((string)$rawFilter->name, (string)$rawFilter->display, (string)$rawFilter->select, $this->getItems($rawFilter));
        }
        return $filters;
    }
    ...
    private function getItems($node)
    {
        ...
        if($node->items) {
            foreach($node->items->item as $rawItem) {
                ...
                $items[] = new FilterItem($name, $display, $weight, $frequency, $image, $this->getItems($rawItem));
            }
        }
        return $items;
    }
